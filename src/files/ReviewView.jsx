...
export default function ReviewView({ weekPlan, meals, items, householdId, onAdvance, refreshItems, syncItems }) {
  ...
  const handleRefresh = async () => {
    const ok = window.confirm(
      "Restart the review?\n\nThis clears your prompt answers and rebuilds the shopping list. Your meal plan stays the same."
    );
    if (!ok) return;
    try {
      await refreshReviewStage(weekPlan.id);
      setPrompts({});
      if (refreshItems) await refreshItems();
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", justifyContent:"flex-end", marginBottom:10 }}>
        <button onClick={handleRefresh} title="Restart the review"
