...
import { updatePlan, getDayOrder, refreshPlanStage } from '../lib/weekPlan';
      ...
      <div style={{ background:"#fff", borderRadius:"20px 20px 0 0", padding:"20px 20px 0",
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search meals…"
          style={{ border:`1.5px solid ${C.line}`, borderRadius:10, padding:"10px 14px",
            fontSize:14, outline:"none", background:"#fff", color:C.ink, colorScheme:"light",
            marginBottom:12 }} />
...
export default function PlanView({ weekPlan, meals, onPlanUpdate }) {
  ...
  const days = getDayOrder(weekPlan.shop_date);

  const handleRefresh = async () => {
    try {
      await refreshPlanStage(weekPlan.id, meals, weekPlan.shop_date);
      // Real-time sub will pick up the new plan, but optimistic update too
      // (refetched via subscribeToWeekPlan in App)
    } catch (err) {
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
        marginBottom:14 }}>
        <span style={{ fontSize:12, color:C.mid }}>Tap a day to swap the suggested meal</span>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          {saving && <span style={{ fontSize:11, color:C.forest }}>Saving…</span>}
          <button onClick={handleRefresh} title="Re-suggest meals"
            style={{ background:C.warm, border:`1px solid ${C.line}`, borderRadius:8,
              padding:"5px 10px", fontSize:11, color:C.mid, cursor:"pointer", fontWeight:600 }}>
            ↻ Re-suggest
          </button>
        </div>
      </div>
