...
import { refreshShopStage } from '../lib/weekPlan';
...
export default function ShopView({ items, refreshItems, weekPlanId }) {
  ...
  const handleRefresh = async () => {
    const ok = window.confirm(
      "Restart the shop?\n\nThis resets all tick marks back to pending. Your list and quantities stay the same."
    );
    if (!ok) return;
    try {
      await refreshShopStage(weekPlanId);
      if (refreshItems) await refreshItems();
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", justifyContent:"flex-end", marginBottom:10 }}>
        <button onClick={handleRefresh} title="Restart the shop"
